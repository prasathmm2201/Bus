...
export default function AdminDroppingPointsPage() {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Are you sure you want to delete this dropping point?")) return;

        setLoading(true);
        const res = await deleteBoardingPointAction(id);
        if (res.success) {
            toast.success("Dropping point deleted successfully");
            fetchData();
        } else {
            toast.error(res.error || "Failed to delete dropping point");
        }
        setLoading(false);
    };
    ...
    const handleSave = async () => {
        ...
        if (res.success) {
            toast.success(editMode ? "Dropping point updated!" : "Dropping point created!");
            setDialogOpen(false);
            fetchData();
            resetForm();
        } else {
            toast.error(res.error || "Failed to save dropping point");
        }
        setLoading(false);
    };
